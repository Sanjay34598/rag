...
def run_real_gemini_tests(rag_service):
    print("\n==================================================")
    print("2. REAL GEMINI RAG TEST (3 Queries)")
    print("==================================================")
    ...
    for i, query in enumerate(test_queries, start=1):
        ...
        print(f"  LLM Mode  : REAL GEMINI (Verified mode: {llm_mode})")
        print(f"  Grounded  : {res['grounded']} (Conf: {res['confidence']})")
        print(f"  Sources   : {len(res['sources'])} chunks")
        print(f"  Latency   : Retrieval={lat['retrieval_ms']}ms | Context={lat['context_ms']}ms | Gemini={lat['llm_ms']}ms | Grounding={lat['grounding_ms']}ms | Total={total_ms:.2f}ms")
...
def run_real_voice_rag_tests(stt_service, rag_service):
    print("\n==================================================")
    print("3. REAL VOICE → SARVAM STT → HYBRID RETRIEVAL → REAL GEMINI TEST")
    print("==================================================")
    ...
    for i, spoken_text in enumerate(voice_queries, start=1):
        ...
        print(f"  LLM Mode    : REAL GEMINI (llm_mode={rag_res.get('llm_mode')})")
        ...
        print(f"  Gemini ms   : {lat['llm_ms']} ms")
...
def run_10_query_real_benchmark(rag_service, stt_service):
    print("\n==================================================")
    print("8. 10-QUERY REAL VOICE & GEMINI BENCHMARK")
    print("==================================================")
    ...
    for i, q in enumerate(queries, start=1):
        ...
        print(f"Query {i:02d}: '{q[:30]}...' | STT={stt_ms:.1f}ms | Ret={lat['retrieval_ms']:.1f}ms | Gemini={lat['llm_ms']:.1f}ms | Total={tot_ms:.1f}ms")
    ...
    print(f"Gemini API Lat   : Avg={llm_avg:.2f}ms | P50={llm_p50:.2f}ms | P70={llm_p70:.2f}ms | P100={llm_p100:.2f}ms")
    print(f"FULL VOICE+RAG   : Avg={tot_avg:.2f}ms | P50={tot_p50:.2f}ms | P70={tot_p70:.2f}ms | P100={tot_p100:.2f}ms")

def main():
    print("==================================================")
    print("STAGE 5C: REAL SARVAM + REAL GEMINI VERIFICATION")
    print("==================================================")
    ...
    run_real_gemini_tests(rag_service)
